...
import { IdentityProvider, IdpGroup } from "@pet-management-webapp/business-admin-app/data-access/data-access-common-models-util";
 ...
 */
export async function controllerCallPatchIdpGroups(session: Session, idpId: string,
    body: IdpGroup[]): Promise<IdpGroup[] | null> {

    const data = (await commonControllerCall(`/api/settings/identityProvider/patchIdpGroups/${idpId}`,
        session, body) as IdpGroup[] | null);
