...
export default async function patchIdpClaims(req: NextApiRequest, res: NextApiResponse) {
    if (req.method !== "POST") {
        notPostError(res);
    }
    ...
    try {
        const fetchData = await fetch(
            `${getOrgUrl(orgId)}/api/server/v1/identity-providers/${idpId}/claims`,
            requestOptionsWithBody(session, RequestMethod.PUT, request)
        );
